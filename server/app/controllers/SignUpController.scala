...
class SignUpController @Inject()(
  ...
  def signUp = silhouette.UnsecuredAction.async(parse.json) {
    implicit request =>
      request.body
        .validate[SignUp]
        .map { signUp ⇒
          ...
          userService.retrieve(loginInfo).flatMap {
            case None =>
              ...
              for {
                ...
                result <- Future.successful(
                  Ok(Json.toJson(Token(authToken.expiry))))
              } yield {
                val url = routes.ActivateAccountController
                  .activate(authToken.tokenId)
                  .absoluteURL()
                mailerClient.send(
                  Email(
            ...
            case Some(user) =>
              /* User already exists */
              Future.successful(
                Conflict(Json.toJson(Bad("signUp.user.alreadyExist"))))
          }
        }
        .recoverTotal {
          case error =>
            Future.successful(
              BadRequest(Json.toJson(Bad(JsError.toJson(error).toString()))))
        }
  }
}
